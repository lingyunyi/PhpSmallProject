...
if(!empty($_SESSION['users']) && !empty($_POST)){
    ...
    if($result != false){
        ...
        $updata = "update adminsdetail set nameX = "."'{$_POST['nameX']}', "."addressX ="."'{$_POST['addressX']}', "."sexX ="."'{$_POST['sexX']}', "."iphoneX ="."'{$_POST['iphoneX']}'"." where identify ="."'{$_SESSION['users']}'";
        global $echo_information;
        mysqli_sql($updata);
        if(mysqli_affected_rows($connect) == 1){
            $echo_information = "更新成功......";
            header('location:../template_admin_info.php?information='."$echo_information");
        }else{
            mysqli_rollback();
        ...
        //否则插入
        $insert =  "insert into adminsdetail(nameX,sexX,addressX,iphoneX,identify) values (" . "'{$_POST["nameX"]}',"."'{$_POST["addressX"]}',"."'{$_POST["sexX"]}',"."'{$_POST["iphoneX"]}', "."'{$_SESSION['users']}' ".")";
        global $echo_information;
        mysqli_sql($insert);
        if(mysqli_affected_rows($connect) == 1){
            //释放一次结果集
