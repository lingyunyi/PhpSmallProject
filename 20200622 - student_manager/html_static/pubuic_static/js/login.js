var Auth = {
	...
	forgot(e) {
		...
		Auth.vars.lowin_login.querySelector('section').setAttribute('action', Auth.vars.option.forgot_url);
		//取消默认实践
		e.preventDefault();
	},
	...
	loginback(e) {
		...
		//获取DOM树中的元素并且设置属性和值
		Auth.vars.lowin_login.querySelector('section').setAttribute('action', Auth.vars.option.login_url);
		//调用类中的setHeight中的值
	...
	init(option) {
		...
		Auth.vars.lowin_login.querySelector('section').setAttribute('action', option.login_url);
